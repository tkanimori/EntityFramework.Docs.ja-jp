...
この記事では、二重、独自のテストを作成するを扱うとしています。 モック作成フレームワークを使用するのには、[テストでモック作成フレームワーク](mocking.md)を参照してください。  

## <a name="testing-with-pre-ef6-versions"></a>EF6 以前のバージョンでのテスト  

この記事で示したコードは、EF6 と互換性が。 EF5 と以前のバージョンをテストするため、[偽のコンテキストでのテスト](http://romiller.com/2012/02/14/testing-with-a-fake-dbcontext/)を参照してください。  
