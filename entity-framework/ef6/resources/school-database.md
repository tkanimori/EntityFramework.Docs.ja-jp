...
このトピックでには、スキーマと School データベースのデータが含まれます。 School サンプル データベースは、Entity Framework のドキュメントのさまざまな場所で使用されます。  

> [!NOTE]
> Visual Studio と共にインストールされているデータベース サーバーは、使用する Visual Studio のバージョンによって異なります。 参照してください[Visual Studio のリリース](~/ef6/what-is-new/visual-studio.md)使用方法の詳細について。  

データベースを作成する手順を次に示します。
