...
たとえば、Fluent API を使用して、スキャフォールディングこの。
...
追加のコンストラクター、メソッド、プロパティなどを追加することもできます。 別のファイルに別の部分クラスを使用します。 このアプローチは、リバース エンジニア リング、モデルをもう一度する予定の場合にも動作します。
