...
これらのツールは、.NET Framework と .NET Core プロジェクトの両方で動作します。
...
ターゲット プロジェクトがすべてのファイルが追加された (または場合によっては削除) します。 ターゲット プロジェクトの既定値、**既定のプロジェクト**パッケージ マネージャー コンソールで、選択されているを使用して指定することもプロジェクト パラメーターです。

スタートアップ プロジェクトは、プロジェクトのコードを実行すると、ツールでエミュレートです。 既定値は 1 つ**スタートアップ プロジェクトとして設定**ソリューション エクスプ ローラーでします。 指定できます - スタートアップ プロジェクトに設定パラメーターを使用します。

一般的なパラメーター:

|                           |                             |
| ------------------------- | --------------------------- |
| コンテキスト\<文字列 >        | 使用する DbContext です。       |
| -プロジェクト\<文字列 >        | 使用するプロジェクトです。         |
| -スタートアップ プロジェクトに設定\<文字列 > | 使用するスタートアップ プロジェクトです。 |
| -Verbose                  | 詳細出力を表示します。        |
...
|        |                                                                       |
| ------ | --------------------------------------------------------------------- |
| フォース | 移行をデータベースに適用されているかどうかをチェックしないでください。 |
...
| コンテキスト\<文字列 >                       | 生成する DbContext の名前。                                    |
| -スキーマ\<String[] >                     | エンティティ型を生成するテーブルのスキーマです。                       |
| -テーブル\<String[] >                      | エンティティ型を生成するテーブル。                                  |
...
| べし等性       | すべての移行でのデータベースで使用できるスクリプトを生成します。 |
