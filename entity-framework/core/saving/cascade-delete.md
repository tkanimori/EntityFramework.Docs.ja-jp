...
EF Core は複数の削除動作を実装しており、個々のリレーションシップの削除動作を構成できます。 また、EF Core は[リレーションシップの必要性](xref:core/modeling/relationships#required-and-optional-relationships)に基づいて、各リレーションシップに対して有用な既定の削除動作を自動的に構成する規則も実装しています。
